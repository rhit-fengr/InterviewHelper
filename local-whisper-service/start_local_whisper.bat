...
set BUNDLED_RUNTIME_EXE=%~dp0runtime\service\local-whisper-service\local-whisper-service.exe
set BUNDLED_PYTHON_EXE=%~dp0runtime\venv\Scripts\python.exe
set RUNTIME_MODE=none
if exist "%BUNDLED_RUNTIME_EXE%" set RUNTIME_MODE=exe
if "%RUNTIME_MODE%"=="none" if exist "%BUNDLED_PYTHON_EXE%" set RUNTIME_MODE=venv

if not "%RUNTIME_MODE%"=="none" goto :runtime_ready
...
if "%WHISPER_VAD_FILTER%"=="" set WHISPER_VAD_FILTER=false
...
if "%RUNTIME_MODE%"=="exe" (
  "%BUNDLED_RUNTIME_EXE%" --host 127.0.0.1 --port %LOCAL_WHISPER_PORT%
) else if "%RUNTIME_MODE%"=="venv" (
  "%BUNDLED_PYTHON_EXE%" -m uvicorn app:app --host 127.0.0.1 --port %LOCAL_WHISPER_PORT%
) else (
